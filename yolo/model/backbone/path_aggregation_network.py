...
class BackboneWithPANet(nn.Module):
    def __init__(self, backbone, depth_multiple):
        super().__init__()
        self.body = backbone 
        ...
        in_channels_list = backbone.out_channels_list
        self.out_channels_list = in_channels_list # set attribute "self.out_channels_list"
        ...
        for i, ch in enumerate(in_channels_list):
            ...
            if i > 0:
                ...
                self.layer_blocks.append(convolutional(ch, in_channels, 1))
                self.upsample_blocks.append(nn.Upsample(scale_factor=2))
                
                self.downsample_blocks.append(convolutional(in_channels, in_channels, 3, 2))
    ...
    def forward(self, x):
        ...
        for i in range(len(x) - 2, -1, -1):
            ...
            last_inner = self.inner_blocks[i](torch.cat((x[i], inner_top_down), dim=1))
